...
public class Boss {
	...
	public Boss(Vector2f pos, int texid, Controller parent, EnemyPath ep, Sprite player, 
		...
		try {
			...
			texture = gp.parseGrid(images.getImage("bullets.png"), 20);
	...
	public void addSprite(Sprite s, int lowesttexid, int highesttexid) {
		me.add(s);
		...
		if(!stopped) {
			for(int i=0; i<playerbullets.size(); i++) {
				for(int j=0; j<myrect.size(); j++) {
					...
					Vector2f p1 = new Vector2f((float)myrect.get(j).getX(), (float)myrect.get(j).getY());
					Vector2f p2 = new Vector2f((float)(myrect.get(j).getX() + myrect.get(j).getWidth()), (float)myrect.get(j).getY());
					Vector2f p3 = new Vector2f((float)(myrect.get(j).getX() + myrect.get(j).getWidth()), (float)(myrect.get(j).getY() - myrect.get(j).getHeight()));
					Vector2f p4 = new Vector2f((float)myrect.get(j).getX(), (float)(myrect.get(j).getY() - myrect.get(j).getHeight()));
					if(Math.abs(Math.sqrt(Math.pow(p1.x - tempos.x, 2) + 
							Math.pow(p1.y - tempos.y, 2))) < (float)25.0f/Display.getHeight() ||
					   Math.abs(Math.sqrt(Math.pow(p2.x - tempos.x, 2) + 
							Math.pow(p2.y - tempos.y, 2))) < (float)25.0f/Display.getHeight() ||
					   Math.abs(Math.sqrt(Math.pow(p3.x - tempos.x, 2) + 
							Math.pow(p3.y - tempos.y, 2))) < (float)25.0f/Display.getHeight() ||
					   Math.abs(Math.sqrt(Math.pow(p4.x - tempos.x, 2) + 
							Math.pow(p4.y - tempos.y, 2))) < (float)25.0f/Display.getHeight()) {
					...
					if(!bullets.get(i).getDestroying()) {
						...
						if(Math.abs(Math.sqrt(Math.pow(p1.x - tempos.x, 2) + 
						   ...
						   Math.abs(Math.sqrt(Math.pow(p4.x - tempos.x, 2) + 
							...
							bullets.get(i).setDestroyingSelf(true);
							explosions.add(new EnemyBullet(bullets.get(i).getPos(), 0.0f));
							explosions.add(new EnemyBullet(new Vector2f((float)(bullets.get(i).getPos().x + (Math.random()*0.1f)-0.05f), (float)(bullets.get(i).getPos().y + (Math.random()*0.04f)-0.02f)), 0.0f));
							explosions.add(new EnemyBullet(new Vector2f((float)(bullets.get(i).getPos().x + (Math.random()*0.1f)-0.05f), (float)(bullets.get(i).getPos().y + (Math.random()*0.04f)-0.02f)), 0.0f));
							parent.damage(5);
						}
					}
