...
private Sprite me;
	...
	public Snake(Controller c, Vector2f pos, int width, int height, ArrayList<Block> blocks, DisplaySetup d) throws IOException {
		this.d = d;
		
		this.parent = c;
		
		left.add(0); right.add(0); still.add(false); texid.add(8); falling.add(0); vely.add(0.0f); mode.add(1);
		stage.add(0);
		this.blocks = blocks;
			...
			if(eyecoords.x > -1.1f && eyecoords.x < 1.1f) {
				...
				if(falling.get(i) == -1) {
					...
					if(mode.get(i) == 1) {
						...
						if(left.get(i) == -1) {
							int tochange = 0;
							if(texid.get(i) > 14 || texid.get(i) < 11) {
								tochange = 11;
								stage.set(i, 1);
							} else {
								if(stage.get(i) == 1) {
