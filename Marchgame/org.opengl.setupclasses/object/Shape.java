...
public class Shape {
	...
	public void save() {
		boolean done = new File(getClass().getResource("").getFile() + name).mkdir();
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(getClass().getResource("").getFile() + 
					name + "/data.obj"));
			for(int i=0;i<nopoints;i++) {
				writer.write("v " + onetriangle.get(i*10) + " "  + onetriangle.get((i*10) + 1)
						 + " "  + onetriangle.get((i*10) + 2));
				writer.newLine();
			}
			for(int i=0;i<nopoints;i++) {
				writer.write("vt " + onetriangle.get((i*10) + 4) + " "  + onetriangle.get((i*10) + 5));
				writer.newLine();
			}
			for(int i=0;i<nopoints;i++) {
				writer.write("vn " + onetriangle.get((i*10) + 6) + " "  + onetriangle.get((i*10) + 7)
						 + " "  + onetriangle.get((i*10) + 8));
				writer.newLine();
			}
			for(int i=0; i<indices.size();i++) {
				for(int j=0; j<indices.get(i).length/3;j++) {
					writer.write("f " + (indices.get(i)[j*3]+1) + "//" + (indices.get(i)[j*3]+1) + " "
							 + (indices.get(i)[(j*3) + 1]+1) + "//" + (indices.get(i)[(j*3) + 1]+1) + " "
							 + (indices.get(i)[(j*3) + 2]+1) + "//" + (indices.get(i)[(j*3) + 2]+1));
					writer.newLine();
				}
			}
			writer.close();
			if(loaded) {
				ImageIO.write(texture, "png", new File(getClass().getResource("").getFile() + 
						name + "/texture.png"));
			} else {
				ImageIO.write(th.getTextureAtlas().getTexture(), "png", new File(getClass().getResource("").getFile() + 
					name + "/texture.png"));
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void draw(ShaderHandler sh, DisplaySetup d) {
		//Setup OPENGL stuff
		if(!made) {
			makeIndices();
		}
