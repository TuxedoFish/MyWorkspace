...
public class ShaderHandler {
	...
	public void addshader(int pId, BufferedReader shaderr, int shadertype) {
		int program = -1;
		int shader = -1;
		...
		if(program == -1) {
			...
			try {
				shader = createShader(shaderr, shadertype);
				programs.get(program - 1).bindshader(shader);
			} catch (Exception e) {
	...
	private int createShader(BufferedReader shaderr, int shadertype) throws Exception{
		//Shader object id
		int shader = 0;
		...
		try{
			...
			ARBShaderObjects.glShaderSourceARB(shader, readFileAsString(shaderr));
			//Compiles the glsl
	...
	private StringBuilder readFileAsString(BufferedReader shader) {
